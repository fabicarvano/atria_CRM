        ...
        updateAtriaTimelineCard: function (timeline) {
            ...
            const stagesHtml = timeline.stages.map((item, index) => {
                ...
                const returnIcon = item.returned
                    ? '<span class="atria-opportunity-stage-return" title="Retorno para este estágio">↩</span>'
                    : '';

                const currentClass = item.isCurrent ? 'is-current' : '';

                return `
                    <div class="atria-opportunity-stage ${currentClass}" data-stage="${this.escapeAtriaHtml(item.stage)}">
