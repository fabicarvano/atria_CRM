...
OUT="/opt/atria/diagnostico_indicadores_oportunidades_$(date +%Y%m%d_%H%M%S).txt"
...
SELECT '=== 5. TEMPO ENTRE ABERTURA E FECHAMENTO, SE EXISTIR CLOSE_DATE ===' AS info;
...
SELECT '=== 8. PROCURANDO TABELAS DE HISTORICO / STREAM / AUDITORIA ===' AS info;
...
SELECT '=== 9. COLUNAS DE TABELAS POSSIVELMENTE RELACIONADAS A HISTORICO ===' AS info;
...
SELECT '=== 10. BUSCANDO REGISTROS DE OPORTUNIDADE NA TABELA NOTE, SE EXISTIR ===' AS info;

SELECT 
    COUNT(*) AS total_notes_opportunity
FROM note
WHERE deleted = 0
  AND (
      parent_type = 'Opportunity'
      OR related_type = 'Opportunity'
      OR target_type = 'Opportunity'
  );

SELECT '=== 11. AMOSTRA DE NOTES DE OPORTUNIDADE ===' AS info;
...
SELECT '=== 12. BUSCANDO POSSIVEIS ALTERACOES DE STAGE EM NOTE.DATA ===' AS info;
